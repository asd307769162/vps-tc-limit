...
一键安装完成后会自动显示识别到的出口网卡，并提示输入限速数值。只需填写数字，例如输入 `15` 就表示 `15 Mbit/s`，不需要填写 `mbit`；直接回车使用当前值，首次安装默认是 `15`。网卡自动识别失败时才会要求手动输入。
...
所有交互提示都会标明回车默认选项。默认配置为：自动网卡、`15 Mbit/s`、`32kb` burst、`400ms` 最大排队延迟。一般只需输入一个限速数字，不需要修改 burst 和 latency 高级参数。
